describe('Get Info About Movie Test', function() {
    var movieService, $httpBackend, scope;
    beforeEach(module('movies-app'));
    beforeEach(inject(function(_aboutMovieService_, _$httpBackend_, $rootScope) {
        movieService = _aboutMovieService_;
    ...
    it('should send get-request', function () {
        ...
        movieService.getMovieInfo(1);
        $httpBackend.flush();
    });

    it('should send correct get-request', function () {
        $httpBackend
            .when('GET', 'https://api.themoviedb.org/3/movie/333333?api_key=2bffc68560bcf99a67d3ea8fa8f937b4&language=ru-RU')
            .respond(200, {test:"testName"});
        movieService.getMovieInfo(333333);
        $httpBackend.flush();
    });
    ...
    it('should return defined promise', function () {
        $httpBackend
            .when('GET', 'https://api.themoviedb.org/3/movie/333333?api_key=2bffc68560bcf99a67d3ea8fa8f937b4&language=ru-RU')
            .respond(200, {test:"testName"});
        var result = movieService.getMovieInfo(333333);
        $httpBackend.flush();
    ...
    it('should return object {test:"testName"} in case of success server response', function () {
        ...
        $httpBackend
            .when('GET', 'https://api.themoviedb.org/3/movie/333333?api_key=2bffc68560bcf99a67d3ea8fa8f937b4&language=ru-RU')
            .respond(200, {test:"testName"});
        movieService.getMovieInfo(333333).then(spyHandler);
        $httpBackend.flush();
    ...
    it('should return error object in case of server response with error', function () {
        ...
        movieService.getMovieInfo(333333).then(null, spyHandler);
        $httpBackend.flush();
